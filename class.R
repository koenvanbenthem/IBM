...
setMethod("initialize","Leprechaun",function(.Object,parent1,parent2,sibs){#parent1 is the mother
  ...
  if(missing(parent1)){
    ...
    .Object@DNAZ[1,]<-pop[[parent1]]@DNAZ[cbind(floor(runif(n=nbLociZ, min=1, max=3)), 1:nbLociZ)]
    .Object@DNAR[1,]<-pop[[parent1]]@DNAR[cbind(floor(runif(n=nbLociR, min=1, max=3)), 1:nbLociR)]
  }
  if(missing(parent2)){
    parent2<-NA
    ...
    .Object@DNAR[2,]<-floor(runif(n = nbLociR, min=1, max=nbAllelesR+1))
      ...
      .Object@DNAZ[2,]<-pop[[parent2]]@DNAZ[cbind(floor(runif(n=nbLociZ, min=1, max=3)), 1:nbLociZ)]
      .Object@DNAR[2,]<-pop[[parent2]]@DNAR[cbind(floor(runif(n=nbLociR ,min=1, max=3)), 1:nbLociR)]
  }
